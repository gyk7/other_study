...
--    인덱스 생성 전, 후 비교했을 때 속도는 약 7배 차이, 쿼리 비용은 약 86000배 차이남.
...
SHOW index FROM gayoung.employees;
	...
	/*(birth_date, first_name) 인덱스 생성했을 때와 (first_name, birth_date) 인덱스 생성했을 때의 속도 차이는 약 15배, 
	쿼리 비용은 약 265배 차이남.*/
...
-- 결론 : 파티션을 만들더라도 쿼리를 제대로 작성하지 않으면 속도, 쿼리 비용에 도움되지 않음.
...
-- 파티션을 제대로 생성하면, 속도가 약 8배 차이, 쿼리 비용은 약 14배 차이남.
